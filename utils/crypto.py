...
def get_aes_key_iv_for_region(region_type, keys_dict):
    ...
    if key_hex and iv_hex:
        try:
            ...
            print(f"{Fore.RED}Error: Failed to get AES key for region {region_type}: {e}{Style.RESET_ALL}")
            exit(-1)
    else:
        print(f"{Fore.RED}Error: Key or IV not found for region {region_type}{Style.RESET_ALL}")
        exit(-1)
...
def aes_cbc_decrypt(encrypted_data, key=None, iv=None):
    ...
    try:
        ...
        print(f"{Fore.RED}AES decryption failed: {e}{Style.RESET_ALL}")
        exit(-1)
...
def decrypt_NTEncode_data(region6_data, offset, key):
    """
